...
public class GeoHotspotLocator {
	...
	public static void main(String[] args) {
		...
		// final JavaPairRDD<Cell, Long> cellAttrs =
		// sc.textFile("yellow_tripdata_2015-01.csv", 100)
		final JavaPairRDD<Cell, Long> cellAttrs = sc.textFile(args[0], 200)
				.mapToPair(line -> new Tuple2<>(createCell(line), 1L)).filter(tuple -> isPointValid(tuple._1()))
		...
		final JavaPairRDD<Cell, Double> getisOrd = cellNetAttrValues
				.mapToPair(a -> calculateGetisOrd(a, broadcastS, broadcastXBar, broadcastN));
		...
		// getisOrd50.coalesce(1).saveAsTextFile("output");
		getisOrd50.coalesce(1).saveAsTextFile(args[1]);
		sc.close();
	}
	...
	private static JavaPairRDD<Cell, Long> calculateCellNetAttrValue(final JavaPairRDD<Cell, Long> cellAttrs) {
		final JavaPairRDD<Cell, Long> neighborAttrValueRdd = cellAttrs
				.flatMapToPair(a -> getCellNeighborAttrValueList(a._1(), a._2()).iterator());
		return neighborAttrValueRdd.reduceByKey((a, b) -> a + b);
	}

	private static Double calculateSValue(final JavaPairRDD<Cell, Long> cellAttrValues, Broadcast<Double> broadcastXBar,
			Broadcast<Integer> broadcastN) {
	...
	private static Tuple2<Cell, Double> calculateGetisOrd(final Tuple2<Cell, Long> getisOrdParameters,
			final Broadcast<Double> broadcastS, final Broadcast<Double> broadcastXBar,
			final Broadcast<Integer> broadcastN) {
		...
		final Double getisOrd = (getisOrdParameters._2() - (broadcastXBar.value() * numberOfNeighbours))
				/ (broadcastS.value() * Math
						.sqrt(((broadcastN.value() * numberOfNeighbours) - (numberOfNeighbours * numberOfNeighbours))
	...
	private static boolean isPointValid(final Cell cell) {
		return cell.getX() >= 0 && cell.getX() <= 39 && cell.getY() >= 0 && cell.getY() <= 54;
	}
